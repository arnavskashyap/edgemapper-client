...
from layers import *
from networks.depth_encoder import *
from networks.depth_decoder import *
...
class Trainer:
    def __init__(self, options):
        ...
        self.models["encoder"] = networks.LiteGfm(model=self.opt.model,
...
#加mask
    ...
    def process_batch(self, inputs):
        ...
        if self.opt.pose_model_type == "shared":
            ...
            #here the outputs.disp=96*320
            outputs = self.models["depth"](features[0])
        else:
            # Otherwise, we only feed the image with frame_id 0 through the depth encoder

            features, kernels = self.models["encoder"](inputs["color_aug", 0, 0])
            outputs = self.models["depth"](features, kernels)
    ...
    def ba_loss(self, pred, target):
        ...
        ba_loss = dx * dy * torch.abs(target - pred)
        ba_loss = torch.mean(ba_loss)
        return ba_loss

    def compute_losses(self, inputs, outputs):
        """Compute the reprojection and smoothness losses and BA_loss for a minibatch
        """
        ...
        for scale in self.opt.scales:
            ...
            # self.opt.disable_automasking、self.opt.avg_reprojection、self.opt.predictive_mask
            if not self.opt.disable_automasking:
            ...
            BA_loss=self.ba_loss(pred,target)
