...
class RepMono(nn.Module):
    ...
    def __init__(self,
                 ...
                 use_se=False,
                 use_hs=True,
                 use_pos_embd_xca=[True, False, False],
                 heads=[8, 8, 8],
                 **kwargs):
        super().__init__()
        if model == 'repmono':
        ...
        dp_rates = [
            x.item()
            for x in torch.linspace(0, drop_path_rate, sum(self.depth))
        ]
        cur = 0
        for i in range(3):
            stage_blocks = []
            for j in range(self.depth[i]):
                if j > self.depth[i] - global_block[i] - 1:
                    if global_block_type[i] == '1':
                        stage_blocks.append(
                            RepTM(dim=self.dims[i],
                                  kernel_size=kernel_size,
                                  stride=stride,
                                  use_se=use_se,
                                  use_hs=use_hs,
                                  hidden_dim=2 * self.dims[i],
